...
export type Doorway = {
  ...
  position: number;
  to: RoomId;
  /** Optional lock predicate; when true, the door is impassable and shows a message. */
...
export type RoomDef = {
  ...
  doorways: Doorway[];
  /** Encounter chance per second on this room's tiles. */
...
export const ROOMS: Record<RoomId, RoomDef> = {
  ...
  'living-room': {
    ...
    doorways: [
      ...
      { side: 'bottom', position: 0.5, to: 'backyard' },
      {
        side: 'right',
        position: 0.5,
        to: 'garage',
  ...
  backyard: {
    id: 'backyard',
    label: 'Backyard',
    floorColor: FLOOR.sky,
    wallColor: WALL.green,
    accentColor: 0xe4f1d4,
    markers: [],
    doorways: [{ side: 'top', position: 0.5, to: 'living-room' }],
    encounterChance: 0.1,
  },
};
